...
export default function MultiLevelListItem({ listItem }) {
  ...
  function recursiveBreadCrumbFunction(item, items) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (_.isEqual(item, element)) {
        let breadCrumbItem = { Id: element.ID, ParentId: element.ParentID, Name: element.Name };
  ...
  return (
        <li className="w-100">
          {listItem.Childs.length == 0 && <NavLink href={`/article/${listItem.ID}/${listItem.Name.replace(" ","_").replace(/ /g,"_")}`} activeClassName="active-border" onClick={() => handleBreadCrumb(listItem)} > {listItem.Name}</NavLink>}
          {listItem.Childs.length > 0 &&
            <>
              <a class="accordion-heading" data-toggle="collapse" data-target={`#submenu${listItem.ID}`} /*aria-expanded="true" */ aria-expanded="false">
                <span class="pull-left"><ChevronLeft size={"16px"} /></span><span class="nav-header-primary">{listItem.Name}</span>
              </a>

              <ul class="nav nav-list collapse child-nav" id={`submenu${listItem.ID}`}>
